...
3. Delete the empty code, and paste in the "Log Vacation Range" script from our [scripts_example.yaml](https://github.com/otw/rto-tracker/blob/main/scripts_example.yaml). **Note:** Remove the top-level ID (e.g., `log_rto_vacation_range:`) and start copying from the `alias:` line!
4. Save the script. Repeat this process for the "Log Office Range" script.
5. Edit your Lovelace dashboard and copy the card configuration from [ui_lovelace_example.yaml](https://github.com/otw/rto-tracker/blob/main/ui_lovelace_example.yaml). Tapping those buttons will now natively pop up the calendars!
...
3. Copy the contents of the [automations_example.yaml](https://github.com/otw/rto-tracker/blob/main/automations_example.yaml) into the editor.
